        ...
        for mention in mentions:
            ...
            if use:
                ...
                if str(msg.id) in config.config["cached"]:
                    msg_dict["archive_url"] = config.config["cached"]["archive_url"]
                    logging.info("tweet {} is cached! sending cached link".format(msg.id))
                    confirm_save(mention.id, config.config["cached"]["telegram_url"][str(msg.id)])
                else:
                    ...
                    if tg is None:
                        ...
                        msg_dict["archive_url"] = tg.caption
